...
/**
 * 4. محرك بحث IMDb ID (للبحث عن الأفلام والمسلسلات)
 */
...
/**
 * 5. مسار التعريب المباشر (Streaming Logs) - يدعم الفارسية/الإنجليزية
 */
app.post("/instant-translate", async (req, res) => {
    ...
    try {
        ...
        sendLog("⏳ جاري تحليل ملف SRT واكتشاف اللغة (فارسي/إنجليزي)...");
        sendLog("🌐 استدعاء محرك الترجمة الذكي (الرجاء الانتظار)...");
        ...
        if (translated && translated.length > 10) {
            ...
            throw new Error("فشل التعريب: المحرك لم يرجع بيانات صالحة.");
        }
    } catch (e) {
...
/**
 * 6. مسار تعديل المزامنة (Offset Sync)
 */
...
/**
 * 8. لوحة التحكم (Stats Page) - الواجهة المفتوحة
 */
...
/**
 * 9. مسارات التشغيل الخلفي (Upload, Save, Delete)
 */
...
app.post("/save-edit", async (req, res) => {
    // دعم الحروف العربية والفارسية والاردية في التحقق
