    ...
    <body>
        <div class="box">
            ...
            <div style="background:#f8f9fa; padding:15px; border-radius:10px; margin-bottom:15px; display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
                <b>🤖 AI:</b> <button onclick="startInstantTranslate()" class="btn" style="background:#8e44ad;">ابدأ التعريب اليدوي</button>
        ...
        <script>
                    ...
                    for (let chunk of chunks) {
                        if (chunk.startsWith('data: ')) {
                            ...
                            if (data.startsWith('[RESULT]')) {
                                const parsed = JSON.parse(data.replace('[RESULT]', ''));
            ...
            async function shiftSync(offset) {
                ...
                document.getElementById('status').innerText = '✅ تم المزامنة';
            }
    ...
    try {
        ...
        <body>
            <div style="max-width: 900px; margin: auto; text-align:center;">
                ...
                <a href="${installUrl}" style="background:#8e44ad; color:white; padding:12px 25px; border-radius:50px; text-decoration:none; font-weight:bold; display:inline-block; margin-bottom:20px;">+ تثبيت الإضافة</a>
                <div class="grid-2">
                    <div class="card">
                        <h3>🔍 البحث (تلقائي)</h3>
                        <input id="q" placeholder="اسم الفيلم..." oninput="autoSearch()"><div id="r"></div>
                    </div>
                    <div class="card">
                        <h3>📤 رفع يدوي</h3>
                        <form action="/upload-manual" method="POST" enctype="multipart/form-data">
                            <select name="type" id="type"><option value="movie">🎬 فيلم</option><option value="series">📺 مسلسل</option></select>
                            <input name="imdbId" id="manual_id" placeholder="tt..." required>
                            <input name="label" id="manual_label" placeholder="اسم النسخة" required>
                            <input type="file" name="subtitleFile" accept=".srt" required onchange="document.getElementById('manual_label').value=this.files[0].name.replace('.srt','')">
                            <button type="submit" style="background:#27ae60; color:white;">حفظ</button>
                        </form>
                    </div>
                </div>
                <div class="card">
                    <table style="width:100%; text-align:right;">
                        <thead><tr><th>المحتوى</th><th>النوع</th><th>الإجراء</th></tr></thead>
                        <tbody>${rows}</tbody>
                    </table>
                </div>
            </div>
            <script>
                async function autoSearch() {
                    const q = document.getElementById('q').value; if(q.length<2) return;
                    const res = await fetch('/search-id?q=' + q); const data = await res.json();
                    document.getElementById('r').innerHTML = data.slice(0,5).map(i => \`<div style="cursor:pointer; padding:5px;" onclick="document.getElementById('manual_id').value='\${i.id}'">\${i.l}</div>\`).join('');
                }
            </script>
        </body></html>`);
...
app.post("/save-edit", async (req, res) => {
    await Subtitle.findOneAndUpdate({ fileId: req.body.fileId }, { arabicText: req.body.newText, isAI: req.body.newText.length > 500 });
